...
pub struct ComputationCache {
    ...
    node: HashMap<NodeHandle, NodeCacheEntry>,
    ...
    fallback: HashMap<TypeId, Box<dyn CacheableFallback>>,
    /// A hash map that stores values generated by a [`FallbackGenerator`].
    ///
    /// Compared to [`Self::fallback`], there can be multiple fallbacks with the same [`TypeId`].
    generated_fallback: HashMap<(NodeHandle, TypeId), Box<dyn CacheableFallback>>,
}
    ...
    pub fn compute_untyped_with(
        ...
        let (computed_results, mut cache) = if let Some(c) = cache {
            (
                &mut c.node,
                Some(Cache {
                    fallback_cache: &mut c.fallback,
                    generated_fallback_cache: &mut c.generated_fallback,
                }),
            )
        } else {
