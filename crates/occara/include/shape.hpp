...
struct Mesh {
  // TODO: This is quite inefficient but works while using autocxx.
  ...
  std::size_t indices_size() const { return indices.size(); }

  std::size_t vertices_size() const { return vertices.size(); }

  std::size_t indices_at(std::size_t index) const { return indices[index]; }

  geom::Point vertices_at(std::size_t index) const { return vertices[index]; }
};
