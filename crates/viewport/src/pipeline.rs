...
pub struct SceneGraph {
    ...
    update_events_in: InputPort<InputEvents>,
...
impl<T> From<SceneGraphBuilder<T>> for SceneGraph {
    fn from(scene_graph: SceneGraphBuilder<T>) -> Self {
        Self {
            ...
            update_events_in: scene_graph.update_node.events_in,
...
impl<T: NodeFactory> ViewportPlugin<T> {
    ...
    pub fn new(node: T) -> Result<Self, ViewportPluginValidationError> {
        validate_plugin(&T::inputs(), &T::outputs()).map(|t| ViewportPlugin(node, t))
    }
}
...
impl DynamicViewportPlugin {
    ...
    pub fn new(node: DynamicNode) -> Result<Self, ViewportPluginValidationError> {
        validate_plugin(node.inputs(), node.outputs()).map(|t| DynamicViewportPlugin(node, t))
    }
}
...
impl ViewportPipeline {
    ...
    /// - `Err(PipelineAddError)` if an error occurred while adding the plugin
    ...
    pub fn add_dynamic_plugin(
        ...
        let handle: NodeHandle = match this
            .graph
            .add_node_dynamic(node.0, handle.node_name.clone())
        {
            Ok(node) => node,
            Err(err) => match err {
                computegraph::AddError::DuplicateName(_) => {
    ...
    fn connect_plugin(
        ...
        match (node_type, self.nodes.last()) {
            ...
            (PluginPosition::Subsequent, Some(prev_node)) => {
                ...
                self.graph
                    .connect_untyped(prev_node.output.clone(), input.clone())
                    .map_err(|e| match e {
                ...
                match self
                    .graph
                    .connect_untyped(prev_node.scene_output.clone().into(), scene_input.clone())
                {
                    Ok(_) => (),
                    Err(e) => match e {
                        computegraph::ConnectError::InputPortAlreadyConnected { .. }
